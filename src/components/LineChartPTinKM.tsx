...
const PTDataVisualizationInKm: React.FC<Props> = ({ data }) => {
    ...
    const transformData = (data: YearlyData): { state: string; values: { year: string; passengers: number; }[] }[] => {
        ...
        states.forEach(state => {
            ...
            for (const year in data) {
                ...
                if (yearDatum) {
                    stateData.values.push({ year, passengers: yearDatum.total_local_passenger_km });
                }
            }
    ...
    useEffect(() => {
        if (data && d3Container.current) {
            ...
            const y = d3.scaleLinear()
                .range([height, 0])
                .domain([0, d3.max(Object.values(data).flat(), d => d.total_local_passenger_km) as number]);
...
export default PTDataVisualizationInKm;
