...
const PTBarChartInKm: React.FC<Props> = ({ data }) => {
    ...
    useEffect(() => {
        if (data && d3Container.current) {
            ...
            const updateChart = (year: string) => {
                ...
                // Sort data
                yearData.sort((a, b) => b.total_local_passenger_km - a.total_local_passenger_km);

                x.domain(yearData.map(d => d.state));
                y.domain([0, d3.max(yearData, d => d.total_local_passenger_km) as number]);

                // Remove existing bars before redrawing
                svg.selectAll(".bar").remove();

                const bars = svg.selectAll<SVGRectElement, TransportData>(".bar")
                    .data(yearData, d => d.state) as d3.Selection<SVGRectElement, TransportData, SVGGElement, unknown>;
                ...
                bars.enter().append("rect")
                    ...
                    .attr("y", d => y(d.total_local_passenger_km))
                    .attr("height", d => height - y(d.total_local_passenger_km))
